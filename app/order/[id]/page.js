        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
              {productName}
              {order.plan_name && <span style={{ color: 'var(--brand)' }}> — {order.plan_name}</span>}
            </p>
          </div>
          ...
          <div style={{
            ...
            {order.plan_name && (
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontWeight: 600 }}>{order.plan_name}</span>
              </div>
            )}
            {order.user_input_data && Object.keys(order.user_input_data).length > 0 && (
              Object.entries(order.user_input_data).map(([key, val]) => (
