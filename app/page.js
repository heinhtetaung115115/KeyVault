    ...
    <>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
        <section style={{ textAlign: 'center', padding: '48px 0 32px' }}>
          <h1 style={{ fontSize: 32, fontWeight: 700, margin: 0, lineHeight: 1.2 }}>{t('hero_title')}</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 520, margin: '12px auto 0', lineHeight: 1.6 }}>
            {t('hero_subtitle')}
          </p>
        ...
        <div className="chips-scroll" style={{ marginBottom: 16 }}>
          <button className={`chip ${activeCategory === 'all' ? 'active' : ''}`} onClick={() => setActiveCategory('all')}>{t('all')}</button>
          {categories.map(cat => (
            <button key={cat.id} className={`chip ${activeCategory === cat.slug ? 'active' : ''}`}
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, overflowX: 'auto' }}>
          {sortOptions.map(opt => (
            <button key={opt.key} className={`tab ${activeSort === opt.key ? 'active' : ''}`}
